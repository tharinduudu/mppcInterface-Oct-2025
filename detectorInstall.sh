...
# - Fetches repo files (mppcInterface/, DataTransfer.sh, Display.sh, dac.py, biasAdj.py, rc.local)
...
# - Installs rc.local to /etc/rc.local + enables rc-local.service
...
# ---- Fetch repo content (INCLUDING rc.local) ----
log "Fetch repo files to ${USER_HOME} (including rc.local)"
sudo -u "${USER_NAME}" bash -lc '
  ...
  # Extract selected files (strip TOPDIR)
...
chown "${USER_NAME}:${USER_NAME}" \
  ...
  "${USER_HOME}/rc.local" || true

# Ensure rc.local has correct perms in home (so install below works cleanly)
chmod 755 "${USER_HOME}/rc.local" || true
...
# ---- Install rc.local and enable rc-local.service ----
if [[ -f "${USER_HOME}/rc.local" ]]; then
  ...
  log "WARNING: ${USER_HOME}/rc.local not found — skipping install."
fi
